...
#include <cstdint>
  ...
  inline unsigned ctz_u64(uint64_t x) {
      unsigned long idx = 0;
      unsigned char nz = _BitScanForward64(&idx, x);
      return nz ? static_cast<unsigned>(idx) : 64u;
  }
#else
  inline unsigned ctz_u64(uint64_t x) {
      return x ? static_cast<unsigned>(ctz_u64(x)) : 64u;
  }
#endif
    ...
    inline size_t row_switch_col(size_t r) const {
        ...
        for(size_t k=0;k<words_per_row;++k){
            ...
            if(w){
                ...
                unsigned tz = (unsigned)ctz_u64(w);
                return (k<<6) + tz;
            }
        }
